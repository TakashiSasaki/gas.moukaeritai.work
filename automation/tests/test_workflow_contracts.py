...
class WorkflowContractTests(unittest.TestCase):
    def test_stage2_runs_after_stage1_only_on_success(self):
        workflow = (REPO_ROOT / ".github" / "workflows" / "stage-2-sync.yml").read_text(encoding="utf-8")
        self.assertIn("github.event.workflow_run.conclusion == 'success'", workflow)
        self.assertIn("github.event_name == 'workflow_dispatch'", workflow)

    def test_stage1_stages_snapshot_retention_deletions(self):
        workflow = (REPO_ROOT / ".github" / "workflows" / "stage-1-inventory.yml").read_text(encoding="utf-8")
