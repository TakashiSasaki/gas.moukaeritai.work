...
function getGeminiAnswersForQuiz(questionsToProcess, selectedModelName, userGenConfig) {
  ...
  const generationConfig = {
    ...
    responseSchema: {
      type: 'array',
      description: 'クイズの質問のリストです。',
      items: {
        type: 'object',
        description: '個々のtrue/false形式のクイズの質問とその解答を表します。',
        properties: {
          question_number: {
            type: 'integer',
          ...
          question_text: {
            ...
            example: 'AIはアーティフィシャル・インテリジェンスの略語で、日本語では人工知能と呼ばれる',
          },
          answer_by_gemini: {
            type: 'boolean',
            description: 'Geminiによって推論されたtrue/false形式の解答です。',
            example: true,
          },
        },
  ...
  const promptInstructions = `以下のJSON形式で提供されるtrue/false問題のリストについて、各問題の正解をtrueかfalseで推論してください。\n提供された各質問に対して、question_number、question_title、question_textはそのまま保持し、推論した解答を answer_by_gemini フィールドに含めて、指定されたJSONスキーマの配列形式で返してください。`;
  const contents = [{
    role: 'user',
...
function testGetGeminiAnswers() {
  const sampleQuestions = [
    { "question_number": 1, "question_title": "Q1", "question_text": "AIはアーティフィシャル・インテリジェンスの略語で、日本語では人工知能と呼ばれる" },
    { "question_number": 2, "question_title": "Q2", "question_text": "AIが急速に発展・普及したことにより、AIという言葉の定義が世界的に確立されたところである" },
    { "question_number": 3, "question_title": "Q3", "question_text": "各データの値を1ずつ大きくすると、データ全体の平均値も1大きくなる。" },
    { "question_number": 4, "question_title": "Q4", "question_text": "データ全体の中央値を上回るデータは、データの値を大きい方から並べたとき真ん中よりも上位にあると言える。" }
  ];
