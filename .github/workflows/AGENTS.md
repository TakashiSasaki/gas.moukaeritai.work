...
This repository uses GitHub Actions to automatically sync Google Apps Script (GAS) projects and maintain meta-information. All workflows now operate on the default branch, **`gas.moukaeritai.work`**.
...
1.  **[a1-update-clasp-list.yml](a1-update-clasp-list.yml)**
    ...
    - **Commit Logic**: Pushes to the default branch if changes are detected in `clasp-list.txt`.
...
2.  **[a2-create_missing_scriptid_dirs.yml](a2-create_missing_scriptid_dirs.yml)**
    ...
    - **Output**:
        - Creates new subdirectories under `projects/`, named by `ScriptID`.
...
graph TD
    subgraph "Chain A: GAS Sync Core"
        A1["a1-update-clasp-list<br/>(Manual)"] -->|workflow_run| A2["a2-create_missing_scriptid_dirs"]
...
#### Execution Steps
1. **Checkout**: Checks out the default branch.
...
6. **Deploy**: If `clasp-list.txt` has changed (meaning a script was added, renamed, or deleted), it commits and pushes the updates to the default branch.
...
- **State Dependency**:
  - Existing subdirectories under `projects/`.
...
#### Execution Steps
1. **Directory Scanning**: Scans known project directories to identify which `ScriptIDs` are already tracked (by reading their `.clasp.json`).
...
4. **Git Push**: Commits the new directories and `.clasp.json` files to the default branch.

#### Generated Artifacts
| Item | Description |
| :--- | :--- |
| `projects/{SCRIPT_ID}/` | A new directory for each newly discovered GAS project. |
| `projects/{SCRIPT_ID}/.clasp.json` | Configuration file enabling `clasp` to know which remote script to pull into this directory. |
...
4. **Git Push**: Commits all code and metadata changes to the default branch.
...
4. **Git Push**: Commits the updated `gas-project-finder.json` to the default branch.
...
4. **Git Push**: Commits all modified `metadata.json` files to the default branch.

#### Generated Artifacts
| File | Format | Description |
| :--- | :--- | :--- |
| `projects/{SCRIPT_ID}/metadata.json` | JSON | Consolidated metadata file for a specific project, combining remote discovery info and local state. |

---

## Maintenance Notes
- All changes are pushed to the default branch by `github-actions[bot]`.
